...
def fetch_non_closed_user_stories(query: NonClosedStoriesQuery) -> str:
    ...
    wiql = "SELECT [System.Id] FROM WorkItems WHERE [System.WorkItemType] = 'User Story' AND [System.State] != 'Closed'"
    wiql_query = {"query": wiql}
    wiql_url = base_url + "wit/wiql?api-version=7.2-preview.2"
...
azureinfo_runnable = azureinfo_prompt | llm.bind_tools(
    [fetch_azure_board_data, fetch_non_closed_user_stories, create_azure_work_item, update_azure_work_item, delete_azure_work_item, CompleteOrEscalate],
    tool_choice="auto"
)
...
def main() -> None:
    ...
    scheduler.add_job(daily_update, 'cron', hour=17, minute=15)
    scheduler.start()
