...
async def process_message(message: str, user_id: str) -> str:
    ...
    # Process the graph and collect the final state
    final_response = ""
    _printed = set()
    for event in multi_agent_graph.stream(state, config, stream_mode="values"):
        _print_event(event, _printed)  # Keep for debugging
        # Extract the latest message from the event
        messages = event.get("messages", [])
        if messages and isinstance(messages, list):
            last_message = messages[-1]
            # Prioritize ToolMessage or AIMessage with meaningful content
    ...
    # If no response was found, invoke the graph one final time to ensure completion
    if not final_response:
        result = multi_agent_graph.invoke(state, config)
        logger.info(f"Final state after invoke: {result}")
    ...
    # Fallback if no meaningful response is found
    if not final_response:
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info(f"Processing initial missing values query for user {user_id}")
    missing_values_response = await process_message(missing_values_query, user_id)
    logger.info(f"Missing values response: {missing_values_response}")
    await update.message.reply_text(missing_values_response)

    # Check for approaching deadlines
    deadline_query = "Check for approaching deadlines"
    logger.info(f"Processing initial deadline query for user {user_id}")
    deadline_response = await process_message(deadline_query, user_id)
    logger.info(f"Deadline response: {deadline_response}")
